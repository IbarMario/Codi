...
$("input[name='cancelar']").click(function(){
    ...
    if(cancelar =='Hospedaje o alimentacion' || cancelar == 'Hospedaje y alimentacion'){
...
function calculo_viaticos(){
    ...
    var tipo_moneda = $('#tipo_moneda').val();
        ...
        $("#agregar_tr").click(function(){
            ...
            var html = '';
            html +='<tr>';
            html +='<td rowspan="3"><select name="id_tipoviaje1['+filas+']" class="required" id="tipo_viaje_id_'+filas+'" onchange="myFunction('+filas+')"><?php echo $opt_tvhtml; ?></select></td>';
            ...
            html +='        Cubre:<br>';
            html +='        <input type="radio" name="cancelar1['+filas+']" value="Hospedaje o alimentacion"> 70%<br>';
            html +='        <input type="radio" name="cancelar1['+filas+']" value="Hospedaje y alimentacion"> 25%<br>';
            html +='        <input type="radio" name="cancelar1['+filas+']" value="Renuncia de viaticos"> 100%';
            html +='</td>';
            html +='<td rowspan="3"><input type="text" name="nro_dia1['+filas+']" size="2" class="required nro_dia1"></td>';
            html +='<td rowspan="3"><input type="text" name="total_pasaje1['+filas+']" size="6" id="total_pasaje_'+filas+'" onchange="calcular_cambio('+filas+')" ><select id="cambio_valor_moneda_'+filas+'" style="display:none;" onchange="calcular_cambio('+filas+')"><option data-type="pr" value="0">Bolivianos</option><option data-type="pr" value="1">Dolares</option></select><br><input type="text" readonly id="resultado_cambio_'+filas+'" value="0" style="display:none;" name="resultado_cambio['+filas+']"/>Bs.</td>';
...
// N Es donde quiero terminar
        ...
        var total = resultado_total();
        document.getElementById('resultado_bs').innerHTML = total.toFixed(2);
        document.getElementById('resultado_us').innerHTML = (total/tipo_cambio.value).toFixed(2);
    ...
    function myFunction(c){
        var tipo_moneda = document.getElementById('cambio_valor_moneda_'+c);
        var tipo_viaje = document.getElementById('tipo_viaje_id_'+c);
        var resultado_cambio = document.getElementById('resultado_cambio_'+c);
        var resultado_son = document.getElementById('resultado_son_'+c);
        var tipo_cambio = document.getElementById('tipo_de_cambio');
        if(tipo_viaje.value == 4 || tipo_viaje.value == 5){
            tipo_moneda.style.display='block';
            resultado_cambio.style.display='block';
            resultado_son.style.display='block';
        } else{
            tipo_moneda.style.display='none';
            resultado_cambio.style.display='none';
            resultado_son.style.display='none';
        }
        var total = resultado_total();
        document.getElementById('resultado_bs').innerHTML = total.toFixed(2);
        document.getElementById('resultado_us').innerHTML = (total/tipo_cambio.value).toFixed(2);
    ...
    function calcular_cambio(c){
        var tipo_moneda = document.getElementById('cambio_valor_moneda_'+c);
        var tipo_cambio = document.getElementById('tipo_de_cambio');
        var total_pasaje = document.getElementById('total_pasaje_'+c);
        var resultado_cambio = document.getElementById('resultado_cambio_'+c);
        var resultado = 0;
        if(tipo_moneda.value == 1){
            resultado = total_pasaje.value * tipo_cambio.value;
        } else{
            resultado = parseFloat(total_pasaje.value);
        }
        resultado_cambio.value = resultado.toFixed(2);
        var total = resultado_total();
        document.getElementById('resultado_bs').innerHTML = total.toFixed(2);
        document.getElementById('resultado_us').innerHTML = (total/tipo_cambio.value).toFixed(2);
    }
    ...
    function resultado_total(){
        var filas = ($("#tabla_tramo tbody tr").length-1)/3;
        var total_bs = 0;
        for(var i = 0;i<filas;i++){
//            console.log(document.getElementById('total_pasaje_'+i).value);
            if(document.getElementById('tipo_viaje_id_'+i).value > 3){
                total_bs += parseFloat(document.getElementById('resultado_cambio_'+i).value);
            } else{
                total_bs += parseFloat(document.getElementById('total_pasaje_'+i).value);
            }
    ...
    <div class="tabs">
        ...
        <div id="editar">
            ...
            <table class="classy" border="1" id="tabla_tipoviaje">
                <tbody>
                    <tr><td>T1 = Intradepartamental</td><td>Tipo de cambio: <input type="text" id="tipo_de_cambio" value="<?php echo $tipo_cambio ?>"></td></tr>
                   <tr><td>T2 = Interdepartamentales</td><td></td></tr>
                   <tr><td>T3 = Franja de Frontera</td><td></td></tr>
                   <tr><td>T4 = Al Exterior - Centro y Sud America y el Caribe</td><td></td></tr>
                   <tr><td>T5 = Al Exterior - Norte America, Europa, Asia, Africa y Oceania</td><td></td></tr>
            </tbody>
        </table>
        <legend>
        ...
        <div ><b id='eliminar_tr' style="cursor: pointer;"> Eliminar <img src="/media/media/images/icons/delete.png"></b></div>
            <table class="classy" border="1" id="tabla_tramo">
                ...
                <?php // echo Form::hidden('tipo_moneda', $pvfucov->tipo_moneda,array('id' => 'tipo_moneda')); }
                    ...
                    <tr>
                        <td rowspan="3"><?php echo Form::select('id_tipoviaje1['.$c.']', $opt_tv, $v->id_tipoviaje, array('class'=>'required','id'=>'tipo_viaje_id_'.$c,'onchange'=>'myFunction('.$c.')')); ?></td>
                        ...
                        <td rowspan="3">
                           ...
                           Cubre:<br>
                            <input type="radio" name="cancelar1[<?php echo $c ?>]" value="Hospedaje o alimentacion" porcentaje="70" <?php if ($v->cancelar == 'Hospedaje o alimentacion') { echo 'checked'; } ?>> 70%<br>
                            <input type="radio" name="cancelar1[<?php echo $c ?>]" value="Hospedaje y alimentacion" porcentaje="25" <?php if ($v->cancelar == 'Hospedaje y alimentacion') { echo 'checked'; } ?>> 25%<br>
                            <input type="radio" name="cancelar1[<?php echo $c ?>]" value="Renuncia de viaticos" porcentaje="0" <?php if ($v->cancelar == 'Renuncia de viaticos') { echo 'checked'; } ?>> 100%

                        </td>
                        <td rowspan="3"><?php echo Form::input('nro_dia1['.$c.']', $v->nro_dia, array('class' => 'required nro_dia1', 'size'=>'2')) ?></td>
                        <td rowspan="3">
                            <?php $mostrar = 'none';
                            if($v->id_tipoviaje == 4 || $v->id_tipoviaje == 5){
                                $mostrar = 'block';
                            } ?>
                            <div style=" width: 150px; height: 80px; border: 0px solid black;">
                                <span style=" float: left; border: 0px solid red;">
                                <?php echo Form::input('total_pasaje1['.$c.']', $v->total_pasaje, array('id' => 'total_pasaje_'.$c, 'size'=>'6','onchange'=>'calcular_cambio('.$c.')')) ?>
                                </span>
                                <span style=" float: left; height: 30px; border: 0px solid red;">
                                    <select id="cambio_valor_moneda_<?php echo $c ?>" data-type="<?php echo $v->total_pasaje; ?>" style=" display: <?php echo $mostrar ?>;float: left;" onchange="calcular_cambio(<?php echo $c ?>)">
                                        <option data-type="pr" value="0">Bolivianos</option>
                                        <option data-type="pr" value="1">Dolares</option>
                                    </select>
                                </span>
                                <div style=" padding-left: 65px;" id="resultado_son_<?php echo $c ?>">
                                    Son : 
                                </div>
                                <span style=" float: left;">
                                    <input type="text" readonly id="resultado_cambio_<?php echo $c ?>" value="<?php echo $v->total_pasaje; ?>" style=" display: <?php echo $mostrar ?>" name="resultado_cambio[<?php echo $c ?>]" size="6"/>
                                </span>
                                <span style=" float: left; border: 0px solid red; padding: 5px; font-size: 1.5em;">
                                Bs.
                                </span>
                            </div>
                        </td>
        ...
        <table class="classy" border="1">
            <thead>
                <tr>
                    ...
                    <th style="text-align:center;" colspan="2" >Total Pasajes</th>
                    <!--<th style="text-align:center;">Total Pasajes $us.</th>-->
                </tr>
            </thead>
            <?php echo Form::hidden('nro_dias', '', array('id' => 'nro_dias')) ?>
            <tbody>
            <?php foreach ($suma_fucov as $v) { ?>
                <tr align='center'>Costo Pasajes
                    ...
                    <td><b><span id="resultado_bs"><?php  echo number_format($v['total_pasaje'],2) ?></span></b> Bs.</td>
                    <td><b><span id="resultado_us"><?php  echo number_format(($v['total_pasaje'] / $tipo_cambio),2) ?></span></b> $us.</td>
                </tr>
            <?php } ?>  
