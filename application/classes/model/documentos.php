...
class Model_Documentos extends ORM{
    ...
    public function contar($text,$entidad)
    {
        ...
        (codigo like '%$text%'
        or nur like '%$text%'
	 or institucion_remitente like '%$text%'
    ...
    {   //d.fecha_creacion between  DATE_FORMAT(CURDATE(), '%Y-%m-01 00:00:00') and CURRENT_TIMESTAMP() AND
        if($entidad==0)
        {
        $sql="SELECT d.id, d.nur, d.cite_original, d.nombre_destinatario, d.cargo_destinatario, d.nombre_remitente,d.cargo_remitente,d.institucion_remitente,d.referencia,d.fecha_creacion, t.tipo        
	 ...
	 OR d.institucion_remitente like '%$text%'
        ...
        {
        $sql="SELECT d.id, d.nur, d.cite_original, d.nombre_destinatario, d.cargo_destinatario, d.nombre_remitente,d.cargo_remitente,d.institucion_remitente,d.referencia,d.fecha_creacion, x.tipo  
    ...
    public function contarDocumentos($id_entidad,$nur,$codigo,$remitente,$cargo_remitente,$destinatario,$cargo_destinatario,$institucion,$referencia,$fecha1,$fecha2)
 ////   public function contarDocumentos($id_entidad,$nur,$codigo,$remitente,$institucion,$referencia,$fecha1,$fecha2)
    {
        $where = '';
        if ($nur<>'') {
        ...
        if ($institucion<>'') {
            $where.=" OR institucion_remitente like '%$institucion%'"; 
        }
        if ($referencia<>'') {
     ...
     public function buscarDocumentos($id_entidad,$nur,$codigo,$remitente,$cargo_remitente,$destinatario,$cargo_destinatario,$institucion,$referencia,$fecha1,$fecha2,$o,$i)
  ////   public function buscarDocumentos($id_entidad,$nur,$codigo,$remitente,$institucion,$referencia,$fecha1,$fecha2,$o,$i)
    {
        $where = '';
        if ($nur<>'') {
            $where.=" OR nur like '%$nur%'"; 
