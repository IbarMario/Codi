...
$pdf->SetAuthor('Ivan Marcelo Chacolla - Efrain Espinoza Callisaya');
...
try {
    ...
    //// Efrain Espinoza Callisaya
    $pdf->SetXY(55,$p);
        $eec = ' - ';
    $pdf->MultiCell(115, 10, 'Remitente: '. $rs->nombre_remitente .$eec . $rs->institucion_remitente, 'TBR', 'L', 0, 1, '', '', true, 0, false, false, 11, 'M');  
    ////$pdf->SetXY(55,$p+10);
  ////  $pdf->MultiCell(115, 10, 'Institucion: '. $rs->institucion_remitente, 'BR', 'L', 0, 1, '', '', true, 0, false, false, 11, 'M');  
    /////////////////////////////////
        
    $pdf->SetXY(55,$p+10);    
    $pdf->MultiCell(115, 10, 'Referencia: '. $rs->referencia, 'BR', 'L', 0, 1, '', '', true, 0, false, false, 11, 'M');    
    ...
    $pdf->MultiCell(115, 10, 'Proveido: '. $rs->proveido, 'BR', 'L', 0, 1, '', '', true, 0, false, false, 11, 'M');
        ...
        $pdf->SetXY(55,$p+30);
        $pdf->MultiCell(115, 10, 'Derivado a: ', '', 'L', 0, 1, '', '', true, 0, false, false, 15, 'M');
        
        $pdf->SetXY(55,$p+30);
        $pdf->Cell(115, 5, $rs->a_oficina,'R',1,'C');  

        $pdf->SetXY(55,$p+35);
        $pdf->SetFont('Helvetica', '', 8); 
        $pdf->Cell(115, 5, $rs->nombre_receptor,'R',0,'C');
