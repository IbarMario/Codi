...
function __construct($dbbea=DATABASEBEA, $host=DBHOST, $user=DBUSER, $pass=DBPASSWORD) {
	$this->BaseDatosbea = $dbbea;
	$this->Servidor = $host;
	$this->Usuario = $user;
	$this->Clave = $pass;	
	$this->connect($dbbea,$host,$user,$pass);
}

function connectbea($dbbea="", $host="", $user="", $pass="")
	{
	if ($dbbea != "") $this->BaseDatosbea = $dbbea;
    ...
    if (!@mysql_select_db($this->BaseDatosbea, $this->Conexion_ID)) {
		$this->Error = "Imposible abrir ".$this->BaseDatosbea ;
...
function nombrecampobea($numcampo) {
return mysql_field_name($this->Consulta_ID, $numcampo);
}
/* Muestra los datos de una consulta */
function verconsultabea() {
		echo "<table border=1>\n";
