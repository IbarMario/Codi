...
// more validations added and uses name instead if div
	...
	var filters = {
			...
			email: function(el) {return /^[A-Za-z][A-Za-z0-9_\.\-]*@[A-Za-z0-9_\-]+\.[A-Za-z0-9_.\-]+[A-za-z]$/.test($(el).val());},
                        web: function(el) {return /^(ht|f)tps?:\/\/\w+([\.\-\w]+)?\.([a-z]{2,4}|travel)(:\d{2,5})?(\/.*)?$/i.test($(el).val());},
			currency: function(el){return /^[0-9,]*$/.test($(el).val());},
			integer: function(el){return /^[0-9"(\-\/\_]*$/.test($(el).val());},
			numeric: function(el){return /^[0-9,]*\.?[0-9]{0,2}$/.test($(el).val());},
...
function loadValidable(){
    ...
    $("form.validable").bind("submit", function(e){        
                ...
                $.each(new String($(el).attr("className")).split(" "), function(x, klass){
                    if ($.isFunction(filters[klass]))
                        if (!filters[klass](el)){
                            //$(el).addClass("reqA");
                            $(el).addClass("req");       
                            //en caso de ser requerido en las etiquetas del formulario
                            if(sw == 1 ) {
                                var iditem = $(el).attr("id");
                                $('body').scrollTo('#'+iditem, 500);
                                sw = 0;
        ...
        if ( $(this).find(".req").size() > 0  || $(this).attr('action')=='' ) {
           ...
           if(confirm("¿Está seguro de los datos?")) {
