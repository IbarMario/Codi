...
$("#pa_empresa").blur( function(){    
   var pe = formatinteger( $("#pa_empresa").val());
   if (pe > 100) {
...
$("#pa_universidad").blur( function(){
   var pu = formatinteger( $("#pa_universidad").val());
   if (pu > 100) {
       alert("  El valor introducido no puede ser mayor a 100%.");
       $("#pa_universidad").val(0);
   }
} );

$("#pa_nacional").blur( function(){
   var pn = formatinteger( $("#pa_nacional").val());
   if (pn > 100) {
       alert("el valor introducido no puede ser mayor a 100%");
       $("#pa_nacional").val(0);
   }
...
$("#pa_importado").blur( function(){
   var pn = formatinteger( $("#pa_importado").val());
   if (pn > 100) {
       alert("el valor introducido no puede ser mayor a 100%");
       $("#pa_importado").val(0);
   } 
...
if (parseFloat(a1+a2)!= 100) {$("#msg4").show();  return false;};
};
if ($("#A_2").val() > 0) { 

    if( a3 > 100 ) { $("#msg2").show(); return false; }
    if( a4 > 100 ) { $("#msg2").show(); return false; }
      if (parseFloat(a3+a4)!= 100) {$("#msg6").show();  return false;};
     };
     if (a1 > 0 || a2 > 0) {    if ($("#A_1").val() == 0) { $("#msg7").show(); return false; }};
    if (a3 > 0 || a4 > 0) { if ($("#A_2").val() == 0) { $("#msg8").show(); return false;} };
