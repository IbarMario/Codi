    ...
    <form class="formA validable" action="acap2cAdd.php" method="post" autocomplete="off" >
      <fieldset> 
            <table width="100%" class="fOpt" >
                <thead>
                <tr>
                    <th align="center">2.3 PRESTACIONES SOCIALES</th>
                ...
                while( $aSueldos = $db->next_record() ) {
                    $estado = $aSueldos["prso_estado"];                    
                ?>                
                <?php if( $aSueldos["indent"] == '1' ) { ?>
                <tr>
                    <td width="71%" class="titR" >1. Aporte Patronal al Seguro de Salud (público o privado)</td>
                    <td width="29%" align="right" ><input type="text" name="A1" id="A1" value="<?php  if ($estado > 0){ echo number_format($aSueldos["prso_valor"]);} else {echo ('');}  ?>" size="20" maxlength="11" class="inpB2 numeric" title ="Registre el monto correspondiente al Seguro de Salud" /></td>
                  </tr>                
                <?php } ?>                
                <?php if( $aSueldos["indent"] == '2'  ) { ?>
                <tr>
                  <td class="titR" >2. Aporte Patronal a las AFP's</td>
                  <td align="right"><input type="text" name="A2" id="A2" value="<?php if ($estado > 0){  echo number_format($aSueldos["prso_valor"]);} else {echo ('');}  ?>" size="20" maxlength="11" class="inpB2 numeric"  title ="Registre el monto correspondiente depositado a las AFP´s sin centavos " /></td>
                </tr>
                <?php } ?>                
                <?php if( $aSueldos["indent"] == '3' ) { ?>
                <tr>
                  <td class="titR" >3. Otros aportes  patronales (especificar)</td>
                  <td align="right"><input type="text" name="A3" id="A3" value="<?php  if ($estado > 0){ echo number_format($aSueldos["prso_valor"]);} else {echo ('');}  ?>" size="20" maxlength="11" class="inpB2 numeric"  title ="Registre el monto correspondiente a otros aportes, Sin centavos " /></td>
                </tr>
                <?php
                ...
                <tr id="otrosdetalle" <?php echo $show1 ?> >
                    <td class="titR" ><input type="text" name="A7" id="A7" value="<?php if ($estado > 0){ echo $aSueldos["prso_descripcion"];;} else {echo ('');}  ?>" size="60" class="inpC2" /></td>
                    <td align="right"></td>
                </tr>
                <?php } ?>                
                ...
                <tr>
                    ...
                    <td align="right"><label class="labB" id="perH"><?php if ($estado > 0){ echo number_format($aSueldos["prso_valor"]) ;} else {echo ('');} ?></label></td>
                </tr>
                <?php } ?>                                              
                <?php } ?>
                </tbody>
