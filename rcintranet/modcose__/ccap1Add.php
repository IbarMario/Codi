...
if( !empty($regisroUID)  ) {      
    ...
    $sql = "SELECT * FROM frm1_ccap1_2aplicaciones WHERE apli_regi_uid= '".$regisroUID."' and apli_delete<>1  ";  
    ...
    if( $db2->numrows() >  0 ) {
        ...
        while( $aCert = $db2->next_record()  ) {
            ...
            $pos = $aCert["apli_position"];            
            $aPosition[$i] = $pos;
            
            // solo modificar aquellos que estan habilitados
                $tapli = OPERATOR::toSql(safeHTML(OPERATOR::getParam("tapli_".$pos)),'Text');
                $napli = OPERATOR::toSql(safeHTML(OPERATOR::getParam("napli_".$pos)),'Text');
                $oapli = OPERATOR::toSql(safeHTML(OPERATOR::getParam("oapli_".$pos)),'Text');
                if ( !empty($tapli) && !empty($napli) && !empty($oapli) ) {
                    $sql3 = "UPDATE frm1_ccap1_2aplicaciones SET apli_valor='".$tapli."',apli_nombre='".$napli."',apli_origen='".$oapli."',apli_suv_uid='".$uid_token."',apli_updatedate=NOW() WHERE apli_position='".$pos."' and apli_regi_uid='".$regisroUID."'";                        
                    $db3->query($sql3);
                }
            $i = $i + 1;
        }
    ...
    for( $j = 1; $j<=$maxrow; $j++ ) {
        ...
        if( ! in_array($j, $aPosition) ) {
            ...
            $tapli = OPERATOR::toSql(safeHTML(OPERATOR::getParam("tapli_".$j."")),'Text');
            $napli = OPERATOR::toSql(safeHTML(OPERATOR::getParam("napli_".$j."")),'Text');
            $oapli = OPERATOR::toSql(safeHTML(OPERATOR::getParam("oapli_".$j."")),'Text');
            
            if ( !empty($tapli) && !empty($napli) && !empty($oapli) ) {
                $sql3 = "INSERT INTO frm1_ccap1_2aplicaciones (apli_regi_uid, apli_valor, apli_nombre, apli_origen, apli_position, apli_suv_uid, apli_delete, apli_createdate, apli_updatedate) VALUES ('".$regisroUID."', '".$tapli."', '".$napli."', '".$oapli."', '".$j."', '".$uid_token."', 0, NOW(), now())";
                $db3->query($sql3);
            }
        }
    }                
